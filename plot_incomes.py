...
correct_labels_top5 = np.load("./data/correct_labels_top5_focal_vgg_7.npy", allow_pickle=True)
incorrect_labels_top5 = np.load("./data/incorrect_labels_top5_focal_vgg_7.npy", allow_pickle=True)
...
# Plot moving average
plt.ylim(0.7,1.0)
# plt.scatter(low_incomes[len(low_incomes)-len(ma_acc):], ma_acc)
plt.plot(low_incomes[len(low_incomes)-len(ma_acc):], ma_acc)

plt.title("Acc v/s Income")
plt.xlabel("Income Level")
plt.ylabel("Accuracy")
plt.savefig(PLOT_DIR+'focal_loss_vgg_7.png', format='png')
plt.show()