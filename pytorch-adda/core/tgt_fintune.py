...
def tgt_finetune(encoder, classifier, train_loader, val_loader):
    # tensorboard configuration
    tb_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),'tensorboard',"tgt")
    ...
    # 1. setup network #
    ####################
    print("[core/pretrain.py] INFO Fineetuning target classifier...")
    ...
    for epoch in range(params.num_epochs_pre):
        ...
        for step, (x_train,y_train) in enumerate(train_loader):
            ...
            tb_logger.add_scalar('Finetuning training Loss', loss.item(), global_step=step_)
        ...
        if ((epoch + 1) % params.save_step_pre == 0):
            save_model(classifier, "tgt-classifier-{}.pt".format(epoch + 1))
        if val_acc_top5 > max_val_acc_top5:
            max_val_acc_top5 = val_acc_top5
            print("[pretrain.py] INFO | Found better model, saving..")
            save_model(classifier, "tgt-classifier-final.pt")

    return encoder, classifier


def eval_finetune(encoder, classifier, val_loader):
