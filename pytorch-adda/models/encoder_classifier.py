...
class Encoder(nn.Module):
    ...
    def __init__(self,num_classes,domain=None,name="resnet18"):
        """Init encoder."""
        ...
        if name == "resnet18":
            ...
            if self.domain == "src":
                print("[encoder_classifier.py] INFO | Creating source encoder,freezing all except last layer in vgg16..")
                for param in model.parameters():
                    param.requires_grad = False
            n_inputs = model.classifier[6].in_features

            model.classifier[6] = nn.Sequential(nn.Linear(n_inputs, 512))
                                            # nn.ReLU(),
        else:
