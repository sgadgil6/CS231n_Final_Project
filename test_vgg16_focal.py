...
def train(model, criterion, optimizer, train_loader, val_loader, n_epochs=40, print_every=2):
    ...
    print('Focal Loss implementation, resnet, gamma = 7')
    ...
    for epoch in range(n_epochs):
        ...
        for x_train, y_train in tqdm(train_loader):
            ...
            #Focal loss
            ce_loss = criterion(output, labels)  # shape (N) #crossentropy loss
            softmax_ = nn.Softmax(dim=1)
            scores_softmax = softmax_(output) #shape (128, 131)
            scores = scores_softmax[np.arange(len(scores_softmax)), labels] #shape (128)
            scores = (1-scores)**gamma
            focal_loss = ce_loss*scores #shape (128)
...
# history, correct_labels_top1, correct_labels_top5, incorrect_labels_top1, incorrect_labels_top5 = train(vgg16_model,
#                                                                                                         criterion,
#                                                                                                         optimizer,
#                                                                                                         dataloaders[
#                                                                                                             'train'],
